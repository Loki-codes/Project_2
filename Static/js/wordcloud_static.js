...
d3.select("#selData").on("change",
  ...
  function stockCloud() {
    ...
    var path = `/Static/data/data.json`
    d3.json(path).then(function (data) {
      ...
      //use the user selection from the event listerner in dataChanged to select the right data
      var filteredData = data[d3.select("#selData").node().value]
      console.log(filteredData)
      filteredData = filteredData.sort((a, b) => b.Counts - a.Counts).slice(0, 50)
